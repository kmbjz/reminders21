...
Пользователь также может создавать задачи (todo) без напоминаний. Задачи отличаются от напоминаний тем, что они не требуют указания времени и не будут отправлять уведомления. Если запрос похож на задачу ("купить цветы в пятницу", "позвонить маме каждый день"), то:
- Установи флаг "is_todo" в true.
- Используй "create" для одноразовых задач или "create_recurring" для повторяющихся задач.
- Для одноразовых задач с указанием дня, установи время на 00:00 указанного дня.

Пользователь также может попросить изменить свой часовой пояс. В этом случае:
- Определи часовой пояс ("timezone"), используя формат IANA, например "Europe/Moscow", "Asia/Yekaterinburg".
- Укажи действие "set_timezone".
- Сгенерируй ответ, например: "Часовой пояс изменен на Europe/Moscow."
...
- Установи флаг "is_todo" в false, если не указано явно, что это задача без напоминания.
